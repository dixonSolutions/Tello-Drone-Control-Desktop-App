...
def attempt_connect():
    ...
    try:
        ...
        try:
            ...
            video_started = True
            try:
                # seed video timestamp to prevent early false disconnect
                last_video_ts = time.time()
            except Exception:
                pass
        except Exception:
...
def periodic_check():
    ...
    if '_sdk_failures' not in globals():
        globals()['_sdk_failures'] = 0
    try:
        now = time.time()
        video_recent = False
        try:
        ...
        # Lightweight SDK probe; tolerate transient failures
        sdk_ok = False
        if tello is not None:
            try:
                _ = tello.get_battery()
        ...
        if sdk_ok:
            ...
            globals()['_sdk_failures'] = globals().get('_sdk_failures', 0) + 1

        # Consider link alive if either video is recent OR SDK probe passes
        if video_recent or sdk_ok:
            is_connected = True
            globals()['_health_failures'] = 0
        else:
            globals()['_health_failures'] = globals().get('_health_failures', 0) + 1
            # Only mark disconnected after sustained failures on both checks
            drop_threshold = 12
            if globals()['_sdk_failures'] >= drop_threshold and globals()['_health_failures'] >= drop_threshold:
                is_connected = False
